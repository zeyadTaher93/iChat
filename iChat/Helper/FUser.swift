...
class FUser {
    ...
    init(_userId: String, _pushId: String?, _createdAt: Date, _updatedAt: Date, _email: String, _firstname: String, _lastname: String, _avatar: String = "", _loginMethod: String, _phoneNumber: String, _city: String, _country: String) {
    ...
    init(_dictionary: NSDictionary) {
        ...
        userId = _dictionary[kOBJECTID] as! String
    ...
    class func registerUserWith(email: String, password: String, firstName: String, lastName: String, avatar: String = "", completion: @escaping (_ error: Error?) -> Void ) {
        ...
        Auth.auth().createUser(withEmail: email, password: password, completion: { (firuser, error) in
            ...
            let fUser = FUser(_userId: firuser!.user.uid, _pushId: "", _createdAt: Date(), _updatedAt: Date(), _email: firuser!.user.email!, _firstname: firstName, _lastname: lastName, _avatar: avatar, _loginMethod: kEMAIL, _phoneNumber: "", _city: "", _country: "")
        ...
        Auth.auth().signInAndRetrieveData(with: credential) { (firuser, error) in
            ...
            fetchCurrentUserFromFirestore(userId: firuser!.user.uid, completion: { (user) in
                ...
                if user != nil && user!.firstname != "" {
                    ...
                    let fUser = FUser(_userId: firuser!.user.uid, _pushId: "", _createdAt: Date(), _updatedAt: Date(), _email: "", _firstname: "", _lastname: "", _avatar: "", _loginMethod: kPHONE, _phoneNumber: firuser!.user.phoneNumber!, _city: "", _country: "")
...
func saveUserToFirestore(fUser: FUser) {
    reference(.User).document(fUser.userId).setData(userDictionaryFrom(user: fUser) as! [String : Any]) { (error) in
...
func userDictionaryFrom(user: FUser) -> NSDictionary {
    ...
    return NSDictionary(objects: [user.userId,  createdAt, updatedAt, user.email, user.loginMethod, user.pushId!, user.firstname, user.lastname, user.fullname, user.avatar, user.contacts, user.blockedUsers, user.isOnline, user.phoneNumber, user.countryCode, user.city, user.country]
